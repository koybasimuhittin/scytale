...
const AllMessages = () => {
  ...
  return (
    <div className="flex mt-40 justify-center items-center">
      {loading ? (
        <p className="w-full text-center text-4xl mt-20">Messages are fetching...</p>
      ) : (
              ...
              {messages.map(value => (
                <Card key={value.messageHash} className="px-4 py-4">
                  <div className="flex flex-col space-y-3">
                    <Label htmlFor={`name${value.messageHash.toString()}`} className="text-xl">
                      Message
                    </Label>
                    ...
                    <div className="flex mt-5 items-center">
                      ...
                      >
                        {value.dataUrl.substring(0, 50) + "..."}
                      </a>
                    </div>
                    <Dialog>
                      <DialogTrigger asChild>
                        {!value.isAlerted && (
                          <Button onClick={() => handleChallenge(value.messageHash)} className="mt-5">
                            Challenge
                          </Button>
                        )}
                      </DialogTrigger>
                      {lastRequest ? (
                        <DialogContent className="sm:max-w-[425px]">
                          ...
                          <DialogFooter>
                            {Number(lastRequest[3] + lastRequest[4]) == 3 && (
                              <Button onClick={() => handleEndRequest(lastRequestId)}>End challlenge</Button>
                            )}
                          </DialogFooter>
                        </DialogContent>
                      ) : (
                        <DialogContent>Waiting for first verification...</DialogContent>
                      )}
                    </Dialog>
                  </div>
                </Card>
